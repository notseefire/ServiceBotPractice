 ...
 * @LastEditors: CYKS
 * @LastEditTime: 2021-12-19 21:15:21
 */
...
TestServer::TestServer(fs::path path): _server_thread(&TestServer::run_server, this) {
  ...
  for (const auto& entry : fs::directory_iterator(path)) {
    ...
    std::string message;
    while(in >> message) {
      BOOST_LOG_TRIVIAL(trace) << boost::format("add message %1%") % message;
...
void TestServer::run_server() {
  ...
  server.Post("/send_msg", [&](const httplib::Request &rep, httplib::Response &res) {
    qq_id id = stoi(rep.get_param_value("user_id"));
    std::string msg = rep.get_param_value("message");

    BOOST_LOG_TRIVIAL(trace) << boost::format("message %1% to %2%") % msg % id;
  });
...
void TestServer::run(int thread_num) {
  ...
  BOOST_LOG_TRIVIAL(info) << "sending all pending message, ready to send stop";
  cli.Get("/stop");
}